...
if($_POST['registro'] == 'guardar'){
    ...
    try {
        $stmt = $conn->prepare('INSERT INTO registrados (nombre_registrado, apellido_registrado, email_registrado, fecha_registro, pases_articulos, talleres_registrados, regalo, total_pagado, pagado ) VALUES (?, ?, ?, NOW(), ?, ?, ?, ?, 0 ) ');
        $stmt->bind_param("sssssis", $nombre, $apellido, $email, $pedido, $registro_eventos, $regalo, $total);
        $stmt->execute();
        $id_insertado = $stmt->insert_id;
        if($stmt->affected_rows) {
...
if($_POST['registro'] == 'actualizar'){
    ...
    try {
        $stmt = $conn->prepare('UPDATE registrados SET nombre_registrado = ?, apellido_registrado = ?, email_registrado = ?, fecha_registro = ?, pases_articulos = ?, talleres_registrados = ?, regalo = ?, total_pagado = ?, pagado = 0 WHERE ID_Registrado = ? ');
...
if($_POST['registro'] == 'abonar'){


    $id_abonar = $_POST['id'];

    try {
        $stmt = $conn->prepare('UPDATE registrados SET pagado = 1 WHERE ID_Registrado = ? ');
        $stmt->bind_param('i', $id_abonar);
        $stmt->execute();
        if($stmt->affected_rows) {
            $respuesta = array(
                'respuesta' => 'exito',
                'id_eliminado' => $id_abonar
            );
        } else {
            $respuesta = array(
                'respuesta' => 'error'
